...
def _fa_bf16(
    ...
    oz = hz // H
    oh = hz % H
    om = sm * BLOCK_M + tl.arange(0, BLOCK_M)
    on = tl.arange(0, BLOCK_N)
    od = tl.arange(0, HEAD_DIM)
    q = tl.load(Q + oz * sqz + oh * sqh + om[:, None] * sqm + od[None, :] * sqk, mask=om[:, None] < N_CTX, other=0.0)
    ...
    for snn in range(0, N_CTX, BLOCK_N):
        k = tl.load(
            K + oz * skz + oh * skh + (snn + on)[:, None] * skn + od[None, :] * skk,
            mask=(snn + on)[:, None] < N_CTX,
            other=0.0,
        )
        if QKB:
        ...
        mij = tl.max(qk, 1)
        mn = tl.maximum(mi, mij)
        al = tl.exp(mi - mn)
        p = tl.exp(qk - mn[:, None])
        li = li * al + tl.sum(p, 1)
        acc = acc * al[:, None]
        v = tl.load(
            V + oz * svz + oh * svh + (snn + on)[:, None] * svn + od[None, :] * svk,
            mask=(snn + on)[:, None] < N_CTX,
            other=0.0,
        )
        if PVB:
        ...
        mi = mn
    tl.store(
        Out + oz * soz + oh * soh + om[:, None] * som + od[None, :] * sok,
...
def test_bf16_flash_attention_never_dispatches_wrong(head_dim, qkb, pvb):
    ...
    ref = torch.softmax((q.float() * (1.0 / math.sqrt(D))) @ k.float().transpose(-2, -1), -1) @ v.float()
...
def _shl_k(a, b, o, N: tl.constexpr):
    i = tl.arange(0, N)
    tl.store(o + i, tl.load(a + i) << tl.load(b + i))
...
def test_shift_by_ge_bitwidth_matches_cuda():
    ...
    a = torch.full((N,), -8, device="mps", dtype=torch.int32)
    o = torch.empty(N, device="mps", dtype=torch.int32)
    _shr_k[(1,)](a, amt, o, N=N)
    torch.mps.synchronize()
...
def _mm_relu_nanprop(a, b, c, M: tl.constexpr, N: tl.constexpr, K: tl.constexpr):
    ...
    on = tl.arange(0, N)
    ok = tl.arange(0, K)
    acc = tl.dot(tl.load(a + om[:, None] * K + ok[None, :]), tl.load(b + ok[:, None] * N + on[None, :]))
...
def test_matmul_epilogue_propagates_nan():
    ...
    _mm_relu_nanprop[(1,)](A, B, C, M=M, N=N, K=K)
    torch.mps.synchronize()
...
def test_scan_then_gather_correct():
    ...
    o = torch.empty(N, device="mps")
    _scan_then_gather[(1,)](a, idx, o, N=N)
    torch.mps.synchronize()
...
def test_split_over_threadgroup_refuses():
    ...
    with pytest.raises(MetalNonRecoverableError):
        _split_kernel[(1,)](a, o0, o1, N=N)
        torch.mps.synchronize()


# --- Re-audit #8 (confirming audit, 2026-06-23): 3 more, all REFUSE now ---------
@triton.jit
def _kloop_where(a, b, c, M, N, K, sam, sak, sbk, sbn, scm, scn, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    pm = tl.program_id(0)
    pn = tl.program_id(1)
...
def test_kloop_matmul_where_epilogue_refuses():
    ...
    c = torch.empty(M, N, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _kloop_where[(1, 1)](a, b, c, M, N, K, *a.stride(), *b.stride(), *c.stride(), BM=M, BN=N, BK=K)
        torch.mps.synchronize()
...
def _fused_rowbias(a, bias, b, c, M: tl.constexpr, N: tl.constexpr, K: tl.constexpr):
    ...
    acc = tl.dot(
        tl.load(a + om[:, None] * K + ok[None, :]),
        tl.load(b + ok[:, None] * N + on[None, :]),
        tl.broadcast_to(bv, (M, N)),
    )
...
def test_fused_row_bias_matmul_refuses():
    ...
    a = torch.randn(M, K, device="mps")
    b = torch.randn(K, N, device="mps")
    bias = torch.arange(M, device="mps", dtype=torch.float32)
    c = torch.empty(M, N, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _fused_rowbias[(1,)](a, bias, b, c, M=M, N=N, K=K)
        torch.mps.synchronize()
...
def test_softmax_nonfp32_output_correct(out_dt):
    ...
    R, NC, BLOCK = 4, 64, 64
    torch.manual_seed(0)
    a = torch.randn(R, NC, device="mps")
    o = torch.empty(R, NC, device="mps", dtype=out_dt)
    _softmax_row[(R,)](a, o, NC, BLOCK=BLOCK)
    torch.mps.synchronize()
...
def test_i32_reduce_exact_above_2_24():
    ...
    N = 256
    a = torch.full((N,), 100000, device="mps", dtype=torch.int32)
    o = torch.empty(1, device="mps", dtype=torch.int32)
    _isum[(1,)](a, o, N=N)
    torch.mps.synchronize()
    assert o.item() == N * 100000, f"i32 sum lost precision: {o.item()} != {N * 100000}"
    a2 = torch.arange(N, device="mps", dtype=torch.int32) + (1 << 25)
...
def _amin(a, o, N: tl.constexpr, K: tl.constexpr):
    i = tl.arange(0, N)
    tl.atomic_min(o + (i % K), tl.load(a + i))


@triton.jit
def _amax(a, o, N: tl.constexpr, K: tl.constexpr):
    i = tl.arange(0, N)
    tl.atomic_max(o + (i % K), tl.load(a + i))
...
def test_fp32_atomic_min_max_correct():
    ...
    torch.manual_seed(1)
    a = torch.randn(N, device="mps")
    o = torch.full((K,), 1e30, device="mps")
    _amin[(1,)](a, o, N=N, K=K)
    torch.mps.synchronize()
    ...
    o2 = torch.full((K,), -1e30, device="mps")
    _amax[(1,)](an, o2, N=N, K=K)
    torch.mps.synchronize()
...
def _aadd64(a, o, N: tl.constexpr):
    i = tl.arange(0, N)
    tl.atomic_add(o + (i * 0), tl.load(a + i))
...
@triton.jit
def _kloop_bias(
    a, bias, b, c, M, N, K, sam, sak, sbk, sbn, scm, scn, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr
):
    pm = tl.program_id(0)
    pn = tl.program_id(1)
    rm = pm * BM + tl.arange(0, BM)
    ...
    for k0 in range(0, K, BK):
        kk = k0 + rk
        acc = tl.dot(
            tl.load(a + rm[:, None] * sam + kk[None, :] * sak), tl.load(b + kk[:, None] * sbk + rn[None, :] * sbn), acc
        )
...
def test_kloop_dot_fused_bias_refuses():
    ...
    c = torch.empty(M, N, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _kloop_bias[(1, 1)](a, bias, b, c, M, N, K, *a.stride(), *b.stride(), *c.stride(), BM=M, BN=N, BK=16)
        torch.mps.synchronize()
...
def _tile_loop_dot(
    ...
    for n0 in range(0, N, BN):
        ...
        acc = tl.dot(
            tl.load(a + rm[:, None] * sam + rk[None, :] * sak), tl.load(b + rk[:, None] * sbk + rn[None, :] * sbn)
        )
        tl.store(c + rm[:, None] * scm + rn[None, :] * scn, acc)
...
def test_tile_loop_dot_with_store_in_body_refuses():
    ...
    c = torch.empty(M, N, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _tile_loop_dot[(1,)](a, b, c, M, N, K, *a.stride(), *b.stride(), *c.stride(), BM=16, BN=16, BK=16)
        torch.mps.synchronize()
...
def _ln_2scalar(X, O, M, N, BLOCK: tl.constexpr):
    ...
    c = tl.arange(0, BLOCK)
    m = c < N
    x = tl.load(X + r * N + c, mask=m, other=0.0)
    mu = tl.sum(x, 0) / N
...
def test_layernorm_two_scalar_args_correct():
    ...
    O = torch.empty(M, N, device="mps")
    _ln_2scalar[(M,)](X, O, M, N, BLOCK=64)
    torch.mps.synchronize()
...
def _nonlooped_acc(a, b, c, M: tl.constexpr, N: tl.constexpr, K: tl.constexpr):
    ...
    acc = tl.dot(
        tl.load(a + om[:, None] * K + ok[None, :]),
        tl.load(b + ok[:, None] * N + on[None, :]),
        tl.full((M, N), 5.0, tl.float32),
    )
    tl.store(c + om[:, None] * N + on[None, :], acc)


@triton.jit
def _kloop_acc(a, b, c, M, N, K, sam, sak, sbk, sbn, scm, scn, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    pm = tl.program_id(0)
    pn = tl.program_id(1)
    rm = pm * BM + tl.arange(0, BM)
    rn = pn * BN + tl.arange(0, BN)
...
def test_3d_reduce_i64_refuses():
    ...
    with pytest.raises(MetalNonRecoverableError):
        _reduce3d_i64[(1,)](a, o, B=B, R=R, C=C)
        torch.mps.synchronize()
...
def test_strided_dot_nonzero_acc_init_refuses():
    ...
    c = torch.empty(32, 32, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _strided_dot_bias[(1, 1)](a, b, c, 32, 32, 32, *a.stride(), *b.stride(), *c.stride(), BM=32, BN=32, BK=16)
        torch.mps.synchronize()
...
@triton.jit
def _strided_pid_bias(a, b, c, sam, sak, sbk, sbn, scm, scn, K, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    pm = tl.program_id(0)
    pn = tl.program_id(1)
    rm = pm * BM + tl.arange(0, BM)
    rn = pn * BN + tl.arange(0, BN)
...
@triton.jit
def _batch_pid_dot(a, b, c, sb, sam, sak, sbk, sbn, scm, scn, K, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    pb = tl.program_id(2)
    pm = tl.program_id(0)
    pn = tl.program_id(1)
    rm = pm * BM + tl.arange(0, BM)
    ...
    for k0 in range(0, K, BK):
        kk = k0 + rk
        acc += tl.dot(
            tl.load(a + pb * sb + rm[:, None] * sam + kk[None, :] * sak),
...
def test_matmul_template_edges_refuse():
    ...
    c = torch.empty(32, 32, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _strided_pid_bias[(1, 1)](a, b, c, *a.stride(), *b.stride(), *c.stride(), 32, BM=32, BN=32, BK=16)
        torch.mps.synchronize()
    a2 = torch.randn(16, 3, device="mps")
    b2 = torch.randn(3, 16, device="mps")
    c2 = torch.empty(16, 16, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _masked_k_dot[(1,)](a2, b2, c2, M=16, N=16, BK=8, K=3)
        torch.mps.synchronize()
    a3 = torch.randn(2, 16, 16, device="mps")
    b3 = torch.randn(16, 16, device="mps")
    c3 = torch.empty(2, 16, 16, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _batch_pid_dot[(1, 1, 2)](
            a3,
            b3,
            c3,
            a3.stride(0),
...
def test_inloop_2d_reduce_underfill_refuses_or_correct():
    ...
    a = torch.randn(T, 8, 16, device="mps")
    o = torch.empty(8, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _inloop_2dreduce[(1,)](a, o, M=8, N=16, T=T)
        torch.mps.synchronize()
    # filled (M*N=512) -> correct
    a2 = torch.randn(T, 8, 64, device="mps")
    o2 = torch.empty(8, device="mps")
    _inloop_2dreduce[(1,)](a2, o2, M=8, N=64, T=T)
    torch.mps.synchronize()
...
def test_noloop_2d_reduce_small_tile_not_over_refused():
    ...
    a = torch.randn(8, 16, device="mps")
    o = torch.empty(8, device="mps")
    _noloop_2dreduce[(1,)](a, o, M=8, N=16)
    torch.mps.synchronize()
...
def test_3d_reduce_fp16_bf16_compiles_correct(dtype):
    ...
    _reduce3d_bf16[(1,)](a, o, B=B, R=R, C=C)
    torch.mps.synchronize()
...
def test_inloop_2d_reduce_fp16_refuses_fp32_ok():
    ...
    for dt in (torch.float16, torch.bfloat16):
        a = torch.randn(T, M, N, device="mps", dtype=dt)
        o = torch.empty(M, device="mps", dtype=dt)
        with pytest.raises(MetalNonRecoverableError):
            _inloop_2dreduce_dt[(1,)](a, o, M=M, N=N, T=T)
            torch.mps.synchronize()
    a2 = torch.randn(T, M, N, device="mps")
    o2 = torch.empty(M, device="mps")
    _inloop_2dreduce_dt[(1,)](a2, o2, M=M, N=N, T=T)
    torch.mps.synchronize()
    torch.testing.assert_close(o2.cpu(), a2.cpu().sum(dim=(0, 2)), rtol=2e-3, atol=2e-3)
...
def test_custom_2value_reduce_refuses():
    ...
    oc = torch.empty(1, device="mps")
    od = torch.empty(1, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _custom_2value[(1,)](a, b, oc, od, N=64)
        torch.mps.synchronize()
...
def _nd_sum(a, o, A: tl.constexpr, B: tl.constexpr, C: tl.constexpr, D: tl.constexpr):
    ...
    x = tl.load(
        a
        + i[:, None, None, None] * B * C * D
        + j[None, :, None, None] * C * D
        + k[None, None, :, None] * D
        + l[None, None, None, :]
    )
    ii = tl.arange(0, A)
    kk = tl.arange(0, C)
    ll = tl.arange(0, D)
    tl.store(o + ii[:, None, None] * C * D + kk[None, :, None] * D + ll[None, None, :], tl.sum(x, 1))
...
def test_nd_nonxor_reduce_refuses_sort_ok():
    ...
    a = torch.randn(2, 2, 2, 2, device="mps")
    o = torch.empty(2, 2, 2, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _nd_sum[(1,)](a, o, A=2, B=2, C=2, D=2)
        torch.mps.synchronize()
    # sort still works (xor ND reduce)
    a2 = torch.randn(2, 16, device="mps")
    o2 = torch.empty(2, 16, device="mps")
    _sort_rows[(1,)](a2, o2, M=2, N=16)
    torch.mps.synchronize()
...
def test_multiprogram_3d_reduce_computes_all_batches():
    ...
    _reduce3d_multiprog[(P,)](a, o, M=M, N=N, K=K)
    torch.mps.synchronize()
...
def _separate_2d(a, os_, om, M: tl.constexpr, N: tl.constexpr):
    ...
    j = tl.arange(0, N)
    x = tl.load(a + i[:, None] * N + j[None, :])
    tl.store(os_ + i, tl.sum(x, 1))
    tl.store(om + i, tl.max(x, 1))
...
def test_combined_2d_reduce_refuses_both_forms():
    ...
    with pytest.raises(MetalNonRecoverableError):
        _combined_2d_noloop[(1,)](a, o, M=M, N=N)
        torch.mps.synchronize()
    a2 = torch.randn(3, M, N, device="mps")
    o2 = torch.empty(M, device="mps")
    with pytest.raises(MetalNonRecoverableError):
        _combined_2d_inloop[(1,)](a2, o2, M=M, N=N, T=3)
        torch.mps.synchronize()
    # separate stores compute correctly (not over-refused)
    os_ = torch.empty(M, device="mps")
    om = torch.empty(M, device="mps")
    _separate_2d[(1,)](a, os_, om, M=M, N=N)
    torch.mps.synchronize()
...
@triton.jit
def _mm_canonical(a, b, c, M, N, K, sam, sak, sbk, sbn, scm, scn, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
...
def test_fast_matmul_n_mod32_byte_exact(dtype, N):
    ...
    os.environ["TRITON_MSL_FAST_MATMUL"] = "1"
    os.environ["TRITON_MSL_COMPILE_SHADER"] = "1"
    M, K = 64, 64
